...
#print dayToHostPairTuple.first()

dayGroupedHosts = dayToHostPairTuple.distinct()
#print dayGroupedHosts.first()

dayHostCount = dayGroupedHosts.map(lambda (x,y) : (x,1))

dailyHosts = dayGroupedHosts.groupByKey().distinct().cache()
#print dailyHosts.first()
print dailyHosts.count()

dailyHostsList = dayHostCount.reduceByKey(lambda x,y : x+y).takeOrdered(dailyHosts.count(), lambda s: s[0])
#print dailyHostsList.first()
...
newDailyHost = dailyHosts.sortByKey()

daysWithHosts = newDailyHost.map(lambda (x,y) : x).collect()
print daysWithHosts
hosts = newDailyHost.map(lambda (x,y) : len(y)).collect()
print hosts
...
dayAndHostTupleDistinct = dayAndHostTuple.distinct()

groupedByDay = dayAndHostTuple.groupByKey()
sortedByDay = groupedByDay.sortByKey()
#print sortedByDay.first()

groupedByDayDistinct = dayAndHostTupleDistinct.groupByKey()
sortedByDayDistinct = groupedByDayDistinct.sortByKey()
#print sortedByDayDistinct.first()

jointDataSet = sortedByDay.join(sortedByDayDistinct).sortByKey()
#print jointDataSet.first()

avgDailyReqPerHost = jointDataSet.map(lambda (day,(connections, hosts)) : (day,len(connections)/len(hosts))).cache()
#print avgDailyReqPerHost.take(30)
...
#print hourCountPairTuple.first()
